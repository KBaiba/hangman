...
//
	public static void main(String[] args) {
		...
		while (playAgain) {
			...
			while ((countGuessed != word.length()) && (countMistakes < 8)) {
				...
				while (!validInput) {
					...
					if (!wrongGuess.isEmpty()) {
						System.out.println("You've already tried: " + wrongGuess);
			...
			System.out.println("");
			System.out.println("If you want to play again, please press 'y'");
			playAgain = false;
			Scanner sc = new Scanner(System.in);
			String playAgainAnswer = sc.next();
			if (playAgainAnswer.contentEquals("y")) {
				playAgain = true;
			}
		}
	}
